...
class QiXin18:
    ...
    def Get_Product_Details(self, Products_Data_Dict:dict):
        product_id = Products_Data_Dict["productId"]
        product_name = Products_Data_Dict["productName"]
        plan_Id = Products_Data_Dict["planId"]
        company_id = Products_Data_Dict["companyId"]
        company_name = Products_Data_Dict["companyName"]

        isDetail = False

        Form_Data = {"productId": product_id, "planId": plan_Id}
            ...
            for Details_Dict in Details_Intetor:
                ...
                for insureAgeFeeDto in iter(insureAgeFeeDtoList):
                    ...
                    for partnerProductFeeItemDto in iter(partnerProductFeeItemDtoList):
                        economyText = partnerProductFeeItemDto["economyText"]
                        feeRateList_1 = partnerProductFeeItemDto["feeRateList"][0]
                        feeRateList_2 = partnerProductFeeItemDto["feeRateList"][1] if len(partnerProductFeeItemDto["feeRateList"])>1 else 0
                        # insert
                        print(product_id, product_name, yearPolicyText, insureAgeText,economyText,feeRateList_1,feeRateList_2)
        else:
            print(product_name, isDetail)
    def run(self):
        while True:
            page = 1
