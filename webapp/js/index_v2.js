...
pagesizeTitle.onclick = function(){
	for(var i=0; i<pagesizeOptions.length; i++){
        pagesizeOptions[i].onclick = function(){
			...
			url_path = "http://120.25.103.152:8002/v2/baoxian/?searchType=1"+ "&program_id="+program_id+"&product_key="+product_key+"&page=1&pageSize="+pageSize;
...
function initPageBtn(){
	...
	for(var i=0;i<pageNumOptions.length;i++){		
        pageNumOptions[i].onclick = function(){
			...
			var pageSize = document.getElementById("dropdownMenu3").value;
			url_path = "http://120.25.103.152:8002/v2/baoxian/?searchType=1"+ "&program_id="+program_id+"&product_key="+product_key+"&page="+pageNo+"&pageSize="+pageSize;
			window.location.href = url_path;
        }
    }
};
